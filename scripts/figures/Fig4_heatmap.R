...
assoc = read_csv("./results/Validation_EWAS/pubmep/2025_11_03_pubmep_associations_between_CpGs_and_outcomes_winsorization.csv") 


assoc = assoc %>%
  dplyr::mutate(pubertal = case_when(stringr::str_detect(predictor, "prepubertal") ~ 0,
                                     TRUE ~ 1)) %>%
  dplyr::filter(pubertal == 1)

outcomes = assoc$outcome %>% unique()
...
# outcomes = names(table(assoc$outcome))
predictors = names(table(assoc2$predictor))

assoc3 = assoc[assoc$outcome %in% outcomes & assoc$predictor %in% predictors, ] 

assoc3 = assoc3 %>%
  dplyr::mutate(outcome = ifelse(stringr::str_detect(outcome, "SBP_zscore"), "SBP_zscore", outcome)) %>%
  dplyr::mutate(outcome = ifelse(stringr::str_detect(outcome, "DBP_zscore"), "DBP_zscore", outcome)) %>%
...
outcomes = assoc$outcome %>% unique() 
outcomes = ifelse(stringr::str_detect(outcomes, "SBP_zscore"), "SBP_zscore", outcomes)
outcomes = ifelse(stringr::str_detect(outcomes, "DBP_zscore"), "DBP_zscore", outcomes)
outcomes = outcomes[match(c("BMI_zscore", "WC_zscore", "FMI", "Glucose_zscore", 
                      "Insulin_zscore", "HOMA_IR_zscore", "QUICKI", 
                      "DBP_zscore", "SBP_zscore", 
                      "TAG_zscore", "HDLc_zscore", "LDLc_zscore"), outcomes)]
